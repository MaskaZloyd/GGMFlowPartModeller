...
[[nodiscard]] ArcBezier arcToBezier(const Vec2& center,
                                    double radius,
                                    double startAngle,
                                    double endAngle) noexcept;

// Build a degenerate Bezier for a straight line segment (w1=1).
[[nodiscard]] ArcBezier segToBezier(const Vec2& start, const Vec2& end) noexcept;

// Evaluate a single rational quadratic Bezier at parameter param ∈ [0, 1].
[[nodiscard]] Vec2 evalRationalQuadratic(const ArcBezier& seg, double param) noexcept;

// Evaluate a rational quadratic Bezier to a polyline of numPoints points.
[[nodiscard]] std::vector<Vec2> evaluateSegment(const ArcBezier& seg, int numPoints) noexcept;
