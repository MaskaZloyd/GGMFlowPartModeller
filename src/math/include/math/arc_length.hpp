...
[[nodiscard]] std::vector<Vec2> resampleArcLength(std::span<const Vec2> polyline,
                                                  int n) noexcept;

// Compute cumulative arc-length values normalized to [0,1].
// Returns vector of same size as input.
[[nodiscard]] std::vector<double> cumulativeArcLength(std::span<const Vec2> polyline) noexcept;
