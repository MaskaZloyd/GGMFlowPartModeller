...
ParamsPanelResult drawParamsPanel(const core::PumpParams& current,
  ...
  if (ImGui::CollapsingHeader("Общие размеры", ImGuiTreeNodeFlags_DefaultOpen)) {
    paramDrag("xa", "xa — осевой отступ, мм", &params.xa, nullptr, nullptr, state,
              "Осевой отступ входного участка");
    paramDrag("dvt", "dvt — диаметр втулки, мм", &params.dvt, &MIN_POSITIVE, nullptr, state,
              "Диаметр втулки (вала)");
    paramDrag("d2", "d2 — диаметр выхода, мм", &params.d2, &MIN_POSITIVE, nullptr, state,
              "Диаметр выхода рабочего колеса");
    paramDrag("b2", "b2 — ширина выхода, мм", &params.b2, &MIN_POSITIVE, nullptr, state,
              "Осевая ширина канала на выходе");
    paramDrag("din", "din — диаметр входа, мм", &params.din, &MIN_POSITIVE, nullptr, state,
              "Диаметр входного патрубка");
  }

  if (ImGui::CollapsingHeader("Дуги втулки", ImGuiTreeNodeFlags_DefaultOpen)) {
    paramDrag("r1", "r1 — радиус дуги 1, мм", &params.r1, &MIN_POSITIVE, nullptr, state,
              "Радиус первой дуги втулки");
    paramDrag("r2", "r2 — радиус дуги 2, мм", &params.r2, &MIN_POSITIVE, nullptr, state,
              "Радиус второй дуги втулки");
    paramDrag("al1", "al1 — наклон выхода, °", &params.al1Deg, &MIN_ZERO, &MAX_ANGLE_POS, state,
              "Угол наклона выходного участка втулки (≥ 0)");
    paramDrag("be1", "be1 — охват дуги 1, °", &params.be1Deg, &MIN_POSITIVE, &MAX_ANGLE_POS, state,
  ...
  if (ImGui::CollapsingHeader("Дуги покрывного диска", ImGuiTreeNodeFlags_DefaultOpen)) {
    paramDrag("r3", "r3 — радиус дуги 3, мм", &params.r3, &MIN_POSITIVE, nullptr, state,
              "Радиус первой дуги покрывного диска");
    paramDrag("r4", "r4 — радиус дуги 4, мм", &params.r4, &MIN_POSITIVE, nullptr, state,
              "Радиус второй дуги покрывного диска");
    paramDrag("al2", "al2 — наклон выхода, °", &params.al2Deg, &MIN_ANGLE_NEG, &MAX_ZERO, state,
              "Угол наклона выходного участка покр. диска (≤ 0)");
    paramDrag("al02", "al02 — наклон горла, °", &params.al02Deg, &MIN_ANGLE_NEG, &MAX_AL02, state,
              "Угол наклона входного участка покр. диска (< 0)");
    paramDrag("be3", "be3 — охват дуги 3, °", &params.be3RawDeg, &MIN_POSITIVE,
              &MAX_ANGLE_POS, state, "Угловой охват первой дуги покр. диска (до корр.)");
